<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.categories.index') }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                    ...
                    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-6" enctype="multipart/form-data">
                        ...
                        <div class="space-y-3">
                            ...
                            @if ($category->image_path)
                                <img src="{{ asset('storage/'.$category->image_path) }}" alt="{{ $category->name }}" class="h-32 w-full rounded-lg object-cover">
                            @endif
